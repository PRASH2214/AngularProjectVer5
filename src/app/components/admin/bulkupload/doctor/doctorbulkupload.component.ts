...
@Component({
  selector: 'app-doctorbulkupload',
  templateUrl: './doctorbulkupload.component.html',
  styleUrls: ['./doctorbulkupload.component.css']
})
export class DoctorBulkuploadComponent implements OnInit {
  ...
  Save() {
    ...
    if (this.finallist != null && this.finallist != undefined) {
      ...
      this._bulkService.savedoctorbulkupload(this.finallist).subscribe((m) => {
  ...
  onFileChange(ev) {
    ...
    reader.onload = (event) => {
      ...
      for (let i = 0; i < firstProp.length; i++) {
        ...
        if (firstProp[i]["Speciality"] == undefined || firstProp[i]["Speciality"] == null || firstProp[i]["Speciality"] == "") {
          this.Notification.showError("Excel is not valid", "");
        ...
        if (firstProp[i]["Department"] == undefined || firstProp[i]["Department"] == null || firstProp[i]["Department"] == "") {
          this.Notification.showError("Excel is not valid", "");
        ...
        if (firstProp[i]["Branch"] == undefined || firstProp[i]["Branch"] == null || firstProp[i]["Branch"] == "") {
          this.Notification.showError("Excel is not valid", "");
          return false;
        }
        ...
        if (firstProp[i]["Email"] == undefined || firstProp[i]["Email"] == null || firstProp[i]["Email"] == "") {
          this.Notification.showError("Excel is not valid", "");

          return false;
        }

        if (firstProp[i]["Doctor Address"] == undefined || firstProp[i]["Doctor Address"] == null || firstProp[i]["Doctor Address"] == "") {
          this.Notification.showError("Excel is not valid", "");
        ...
        if (firstProp[i]["Doctor License Number"] == undefined || firstProp[i]["Doctor License Number"] == null || firstProp[i]["Doctor License Number"] == "") {
          this.Notification.showError("Excel is not valid", "");
          return false;
        }
        ...
        var objhostpital = {
          ...
          "specialityName": firstProp[i]["Speciality"],
          "departmentName": firstProp[i]["Department"],
          "branchName": firstProp[i]["Branch"],
          ...
          "doctorAddress": firstProp[i]["Doctor Address"],
          "doctorLicenseNumber": firstProp[i]["Doctor License Number"],
      ...
      if (this.sendhospitaljson != null) {
        this._bulkService.validatedoctorbulkupload(this.sendhospitaljson).subscribe((m) => {
          if (m.success == true && m.status == 1) {
            this.saveshow = true;
  ...
  checkdata() {
    ...
    if (this.errorlist.length > 0 && this.errorlist != null) {
      this._bulkService.validatedoctorbulkupload(this.errorlist).subscribe((m) => {
        if (m.success == true && m.status == 1) {
