...
@Component({
  selector: 'patientmrsummary',
  templateUrl: 'patientmrsummary.component.html'
})
export class PatientMRSummaryComponent {
  ...
  search(Datefrom, DateTo, hosppitalddl) {
    ...
    if (this.dispreporttype == 1) {
      ...
      this._srv.getpatientappointmentdetailbyadmin(sendjson).subscribe((m) => {
      ...
      this._srv.getmrappointmentdetailbyadmin(sendjson).subscribe((m) => {
