...
@Component({
  selector: "app-paitentlayout",
  templateUrl: "./paitentlayout.component.html",
  styleUrls: ["./paitentlayout.component.scss"],
})
export class PatientlayoutComponent implements OnInit {
  ...
  constructor(
    ...
    private prservice: PatientProfileService
  ) { }

  ngOnInit(): void {
    this.storage.add("Utype", 3);
    this.globalx.CallRequest(this.CallRequest.bind(this));

    this.master.CurrentUser = this.storage.get("listoken");
    var loginId = this.master.CurrentUser.model.patientId;
    var name = this.master.CurrentUser.model.patientName;
    ...
    this.signalRService.startConnection("P" + loginId, name);
  ...
  showswal(data) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.connectionService.callaccepted(this.master.patientConnectionId, this.master.doctorConnectionId, data.message).subscribe((m) => {

        }, err => {

        });
        this.nav.navigateByUrl('/patient/consultationroom');
